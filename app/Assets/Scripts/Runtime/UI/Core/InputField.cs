...
namespace Cuboid.UI
{
    ...
    ///
    /// Adapted from the TextMeshPro package, in order to add support for a custom
    /// numeric input keyboard.
    ///
    /// Also to make it compatible with VR. (allowing the caret to be set)
    /// 
    /// When the content type is Decimal or Integer, we want to open the custom
    /// numeric input keyboard.
    ///
    /// For all other content types we want to open the Oculus system keyboard. 
    ///
    /// The keyboard is opened using the TouchScreenKeyboard.Open("", TouchScreenKeyboardType.Default)
    /// call. However, with the system keyboard, only the default type is supported. 
    /// https://developer.oculus.com/documentation/unity/unity-keyboard-overlay/
    ///
    /// We also want to display a preview of the text that is being input by the user above the
    /// keyboard. This, because when opening the system keyboard, it will trigger the OVRManager.InputFocusLost
    /// event, which causes UI and controllers to be disabled.
    ///
    /// So, text selection when inputting a value is not possible. 
    /// </summary>
        ...
        IScrollHandler
    {
        ...
        public enum CharacterValidation
        {
            ...
            Integer,
            Decimal,
            Calculated, // Cuboid add ( ) * + - / characters
            Alphanumeric,
            Name,
            Regex,
            EmailAddress,
        ...
        {
            get { return inputSystem != null ? inputSystem.compositionString : UnityEngine.Input.compositionString; }
        }
        ...
        private Action<char> _onVirtualKeyboardTextInput;
        private Action _onVirtualKeyboardBackspace;

        [SerializeField] private bool m_CantLoseFocus = false;

        #region Actions

        /// <summary>
        /// Called by the virtual keyboard when done editing
        /// </summary>
        public Action<string> OnConfirmValue;

        /// <summary>
        /// Called when changing values
        /// </summary>
        ...
        protected override void Start()
        {
            ...
            _virtualKeyboard = VirtualKeyboard.Instance;
            _onVirtualKeyboardTextInput = OnVirtualKeyboardTextInput;
            _onVirtualKeyboardBackspace = OnVirtualKeyboardBackspace;
        ...
        private void Register()
        {
            ...
            {
                _virtualKeyboard.OnTextInput += _onVirtualKeyboardTextInput;
                _virtualKeyboard.OnBackspace += _onVirtualKeyboardBackspace;
            }
        }
        ...
        /// <summary>
        /// https://forum.unity.com/threads/how-to-send-simulated-keyboard-events-to-an-inputfield-from-buttons-click-handlers.307927/
        /// </summary>
        /// <param name="character"></param>
        /// <returns></returns>
        private Event GetKeyboardEvent(char value)
        {
            Event fakeEvent;
            switch (value)
            {
                ...
                case '#':
                    ...
                    break;
                default:
                    fakeEvent = Event.KeyboardEvent(value.ToString());
                    char keyChar = value;
        ...
        private void OnVirtualKeyboardTextInput(char character)
        {
            HandleVirtualKeyboardInput(GetKeyboardEvent(character));
        }

        private void OnVirtualKeyboardBackspace()
        {
            HandleVirtualKeyboardInput(Event.KeyboardEvent("backspace"));
        }

        private void HandleVirtualKeyboardInput(Event keyboardEvent)
        {
            if (!isFocused)
                return;

            EditState shouldContinue = KeyPressed(keyboardEvent);
        ...
        // Cuboid changed
        public bool shouldHideMobileInput => m_HideMobileInput;

        // Cuboid changed. For fucks sake. why so many checks. 
        public bool shouldHideSoftKeyboard => m_HideSoftKeyboard;
        ...
        protected virtual void LateUpdate()
        {
            ...
            //return;
            // Cuboid changed: we don't use the soft keyboard anymore so we can safely disable this. 
        ...
        public virtual void OnUpdateSelected(BaseEventData eventData)
        {
            ...
            while (Event.PopEvent(m_ProcessingEvent))
            {
                ...
                //Debug.Log($"{m_ProcessingEvent.rawType} Got event");
        ...
        protected void UpdateLabel()
        {
            if (m_TextComponent != null && m_TextComponent.font != null && m_PreventCallback == false)
            {
                ...
                m_TextComponent.text = IsEditingMultiple ? "Multiple" : processed + "\u200B"; // Extra space is added for Caret tracking.
                ...
                if (m_LineLimit > 0)
                {
                    ...
                    if (textInfo != null && textInfo.lineCount > m_LineLimit)
                    {
                        ...
                        m_TextComponent.text = IsEditingMultiple ? "Multiple" : text + "\u200B";
                    }
                }
        ...
        protected char Validate(string text, int pos, char ch)
        {
            ...
            // Cuboid added
            else if (characterValidation == CharacterValidation.Calculated)
            {
                // Integer or decimal with ( ) * + - / characters added
                if (ch >= '0' && ch <= '9') return ch;
                if (ch == '-' || ch == '+' || ch == '(' || ch == ')' || ch == '*' || ch == '/') return ch;

                var separator = Thread.CurrentThread.CurrentCulture.NumberFormat.NumberDecimalSeparator;
                if (ch == Convert.ToChar(separator)) return ch;
            }
        ...
        private void ActivateInputFieldInternal()
        {
            ...
            // Cuboid added
            if (shouldHideSoftKeyboard == false && (keyboardType == TouchScreenKeyboardType.NumberPad || keyboardType == TouchScreenKeyboardType.NumbersAndPunctuation))
            {
                // open the custom number keyboard.
                NumericKeyboardPopup popup = PopupsController.Instance.OpenPopup<NumericKeyboardPopup>(PopupsController.Instance.NumericKeyboardPopupPrefab, false);
                // set the value of the popup
                // set the callbacks
                popup.SetSourceInputField(this);
            }
            else if (shouldHideSoftKeyboard == false)
            {
                // this means we instantiate the popup
                FullKeyboardPopup popup = PopupsController.Instance.OpenPopup<FullKeyboardPopup>(PopupsController.Instance.FullKeyboardPrefab, false, new PopupsController.PopupParams()
                {
                    ClickingOutsidePopupCloses = false,
                    DontInterceptUndoButtons = true,
                    WorldSpace = true
                });
                popup.SetSourceInputField(this);
            }
            // we don't want to use the normal soft keyboard anymore
//            else if (TouchScreenKeyboard.isSupported && shouldHideSoftKeyboard == false)
//            {
...
//                m_TouchKeyboardAllowsInPlaceEditing = true;//TouchScreenKeyboard.isInPlaceEditingAllowed;
//#endif
//            }
            else
            {
                // Cuboid: this is the place where we need to make sure that it does allow editing in place.
                // because this is what gets called when shouldHideSoftKeyboard is true. 
        ...
        private void EnforceContentType()
        {
            switch (contentType)
            {
                ...
                // Cuboid added
                case ContentType.Calculated:
                    {
                        m_LineType = LineType.SingleLine;
                        m_InputType = InputType.Standard;
                        m_KeyboardType = TouchScreenKeyboardType.NumbersAndPunctuation;
                        m_CharacterValidation = CharacterValidation.Calculated;
                        break;
                    }
